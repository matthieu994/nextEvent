...
export default class ModifyPayment extends Component {
    ...
    constructor (props){
      ...
      this.state = {
        ...
        this.state.spent.to.map((item, i) => (
          this.state.selection.push({ name : this.state.spent.to[i], 
    ...
    shareAmount = () => {
      let count = 0;
      this.state.selection.map((item, i) => (count = item.checked ? count+1 : count)),

      share = this.state.spent.amount / count,
      tmp = this.state.selection,

      this.state.selection.map((item, i) => (tmp[i].owe = item.checked ? share : 0))
                  ...
                  this.state.selection.map((item, i) => (
                    <ListItem
                      ...
                      <CheckBox
                        ...
                        containerStyle={{marginLeft: 0, backgroundColor : 'rgb(210, 225, 230)'}}                   
                      />}
                      rightElement={<Text>{item.owe}</Text>}
                       
                      containerStyle={{padding : 0, backgroundColor : 'rgb(232, 243, 250)', marginBottom : 10}}
