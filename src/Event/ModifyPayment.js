...
export default class ModifyPayment extends Component {
  ...
  constructor(props) {
    ...
    this.payment = props.navigation.state.params.properties
  ...
  componentDidMount() {
    ...
    this.setState({ event }, () => {
      ...
      Object.keys(this.state.event.properties.users).map(email => {
        const current = this.payment.to.find(u => u.email === email)

        selection.push({
          email,
          user: this.state.event.properties.users[email],
          owe: current ? current.owe : 0,
          checked: !!current
        })
      })
...
ModifyPayment.contextType = UserContext
